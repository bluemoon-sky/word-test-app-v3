...
type Props = {
    words: Word[];
    onFinishStudy: () => void; // 학습 완료 후 테스트로 넘어갈 콜백
};
...
export default function WordStudy({ words, onFinishStudy }: Props) {
    ...
    return (
        <div className="max-w-md mx-auto">
            ...
            <div
                onClick={handleFlip}
                className="bg-white rounded-3xl shadow-xl border-4 border-indigo-200 p-8 min-h-[280px] flex flex-col items-center justify-center cursor-pointer select-none transition-all hover:shadow-2xl hover:border-indigo-300 active:scale-[0.98]"
            >
                <div className="text-center">
                    ...
                    {!showMeaning && currentWord.pronunciation && (
                        <p className="text-slate-400 font-medium text-lg">[{currentWord.pronunciation}]</p>
                    )}
                </div>
            ...
            <div className="flex gap-3 mt-4">
                ...
                {isLast ? (
                    <button
                        ...
                        className="flex-1 py-3.5 bg-gradient-to-r from-orange-400 to-amber-500 text-white font-bold rounded-2xl flex items-center justify-center shadow-lg shadow-orange-500/30 hover:from-orange-500 hover:to-amber-600 transition-all"
                    >
                        🚀 테스트 시작하기!
                    </button>
                ) : (
